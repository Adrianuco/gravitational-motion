...
function setup() {
  ...
  // inicializamos los formularios y botones
  inicializarFormularioCuerpo();
  inicializarPlanetasPredeterminados();
  inicializarFormularioEstrella();

  const canvasClick = document.getElementById("defaultCanvas0");
  ...
  // instanciamos el universo y la estrella central
  universo = new Universo();
  sol = new CuerpoCeleste("Sol", 1.989e30, 0, 0, 0, 0, 30, "#ffcc00");

  // agregamos la estrella al universo y actualizamos la escala
  universo.agregarCuerpo(sol);
  actualizarEscala();
}

function actualizarEscala() {
  // verificamos que el universo y sus cuerpos existan
  if (!universo || universo.cuerpos.length === 0) return;

  // iteramos sobre los cuerpos para encontrar la distancia máxima desde el origen
  let maxDist = 0;
...
function draw() {
  ...
  if (cardTiempo) {
    cardTiempo.textContent = `Tiempo: ${universo.t / 86400} días`;
  }
  ...
  // dibujar vectores si está pausado
  if (universo.pausado) {
    universo.calcularFuerzas(); // calculamos fuerzas en el momento de la pausa
